...
from .serializers import AjoutFournisseurSerializer, GetSerializer
from App.models import Fournisseur


class Crud(APIView):
    def post(self, request):
        seria = AjoutFournisseurSerializer(data=request.data)
        print(seria.is_valid())
        if seria.is_valid():
            fournisseur = seria.save()
            seria = GetSerializer(fournisseur)
            return Response(status=status.HTTP_201_CREATED, data = seria.data)
